...
class DocumentImpl extends NodeImpl {
  ...
  setParsingBlockingScript(script) {
    if (this._pendingParsingBlockingScript) {
      throw new Error("Internal error: Document already has parsing blocking script");
    }

    this._pendingParsingBlockingScript = script;
  }

  parsingBlockingScriptLoaded(script) {
    if (this._pendingParsingBlockingScript !== script) {
      throw new Error("Internal error: Unexpected parsing blocking script");
    }

    this.currentParser.resumePendingParsingBlockingScript(script);
  }

  addAsapScript(script) {
    this._scriptsThatWillExecuteASAP.add(script);
  }

  // https://html.spec.whatwg.org/multipage/#the-end step 5.
  asapScriptLoaded(script) {
    this._scriptsThatWillExecuteASAP.delete(script);
  ...
  finishLoad() {
    ...
    this._processDeferredScript();
  }

  // https://html.spec.whatwg.org/multipage/#the-end step 3-4.
  _processDeferredScript() {
    // TODO
