...
const { shadowDomIncludingInclusiveDescendantIterator } = require("../helpers/shadow-dom");
const { CUSTOM_ELEMENT_REACTION_STACK } = require("../helpers/custom-elements");
...
class DocumentImpl extends NodeImpl {
  ...
  createElement(localName, options) {
    ...
    return createElement(this, localName, namespace, null, is, true);
  }
  ...
  createElementNS(namespace, qualifiedName, options) {
    ...
    return createElement(this, extracted.localName, extracted.namespace, extracted.prefix, is, true);
  }
  ...
  _adoptNode(node) {
    ...
    if (oldDocument !== newDocument) {
      for (const inclusiveDescendant of shadowDomIncludingInclusiveDescendantIterator(node)) {
        inclusiveDescendant._ownerDocument = newDocument;
      }

      for (const inclusiveDescendant of shadowDomIncludingInclusiveDescendantIterator(node)) {
        if (inclusiveDescendant.nodeType === NODE_TYPE.ELEMENT_NODE && inclusiveDescendant._isCustom()) {
          CUSTOM_ELEMENT_REACTION_STACK.enqueueCECallbackReaction(inclusiveDescendant, "adoptedCallback", [
            oldDocument,
            newDocument
          ]);
        }
      ...
      for (const inclusiveDescendant of shadowDomIncludingInclusiveDescendantIterator(node)) {
        if (inclusiveDescendant._adopted) {
          inclusiveDescendant._adopted(oldDocument);
        }
      }
    }
  }
