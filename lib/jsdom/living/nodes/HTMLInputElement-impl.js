...
const convertStringToDateByTypeMap = new Map([
  [
    "date", input => {
      const parsedInput = convertStringToNumberByTypeMap.get("date")(input);
  ...
  [
    ...
    "month", input => {
      ...
      date.setUTCMonth(parsedMonthString.month);
      return date;
    }
  ],
  [
    "week", input => {
      const parsedInput = convertStringToNumberByTypeMap.get("week")(input);
      return isNaN(parsedInput) ? NaN : new Date(parsedInput);
    }
  ],
  [
    "time", input => {
      const parsedInput = convertStringToNumberByTypeMap.get("time")(input);
      return isNaN(parsedInput) ? NaN : new Date(parsedInput);
    }
  ],
  [
    "datetime-local", input => {
      const parsedInput = convertStringToNumberByTypeMap.get("datetime-local")(input);
      return isNaN(parsedInput) ? NaN : new Date(parsedInput);
    }
  ]
]);

const serializeDateByTypeMap = new Map([
  [
    "date", input => {
...
const convertNumberToStringByTypeMap = new Map([
  [
    // https://html.spec.whatwg.org/multipage/input.html#date-state-(type=date):concept-input-value-string-number
    "date", input => serializeDateByTypeMap.get("date")(new Date(input))
  ],
  [
  ...
  [
    ...
    "datetime-local", input => serializeDateByTypeMap.get("datetime-local")(new Date(input))
  ],
...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  get _convertNumberToString() {
    return convertNumberToStringByTypeMap.get(this.type);
  }

  get _convertDateToString() {
    return serializeDateByTypeMap.get(this.type);
  }

  get _convertStringToDate() {
    return convertStringToDateByTypeMap.get(this.type);
  }
  ...
  get valueAsDate() {
    ...
    const stringToDateConverter = this._convertStringToDate;
    const window = this._ownerDocument._defaultView;
    const convertedValue = stringToDateConverter(this._value);
  ...
  set valueAsDate(v) {
    if (!allowValueAsDate(this.type)) {
      throw new DOMException("Failed to set the 'valueAsDate' property on 'HTMLInputElement': This input element " +
        "does not support Date values.", "InvalidStateError");
    }

    if (v !== null && Object.prototype.toString.call(v) !== "[object Date]") {
      throw new TypeError("Failed to set the 'valueAsDate' property on 'HTMLInputElement': The provided value is " +
        "not a Date.");
    }

    if (v === null || (Object.prototype.toString.call(v) === "[object Date]" && isNaN(v))) {
      this._value = "";
    }

    const convertDateToString = this._convertDateToString;
    this._value = convertDateToString(v);
  }
  ...
  set valueAsNumber(v) {
    ...
    if (!allowValueAsNumber(this.type)) {
      throw new DOMException("Failed to set the 'valueAsNumber' property on 'HTMLInputElement': This input element " +
        "does not support Number values.", "InvalidStateError");
    }

    const convertNumberToString = this._convertNumberToString;
    this._value = convertNumberToString(v);
  }
  ...
  _stepUpdate(n, isUp) {
    ...
    if (!allowValueAsNumber(this.type)) {
      throw new DOMException(`Failed to invoke '${isUp ? "stepUp" : "stepDown"}' method on 'HTMLInputElement': This ` +
        "input element does not support Number values.", "InvalidStateError");
    }

    const allowedValueStep = this._allowedValueStep;
    if (allowedValueStep === null) {
      throw new DOMException(`Failed to invoke '${isUp ? "stepUp" : "stepDown"}' method on 'HTMLInputElement': This ` +
        "input element does not support value step.", "InvalidStateError");
    }
    ...
    if (min !== null && value < min) {
      // should be value = this._stepAlign(min, true);
      value = min;
    }

    if (max !== null && value > max) {
      // should be value = this._stepAlign(max, false);
      value = max;
    }
