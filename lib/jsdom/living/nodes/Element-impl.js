...
class ElementImpl extends NodeImpl {
  ...
  _attach(rootNode) {
    if (rootNode === this._ownerDocument) {
    ...
    if (this._shadowRoot) {
      this._shadowRoot._attach(this._shadowRoot);
    }

    super._attach(rootNode);
  }

  _detach(rootNode) {
    super._detach(rootNode);

    if (this._shadowRoot) {
      this._shadowRoot._detach(this._shadowRoot);
    }
  ...
  setAttribute(name, value) {
    ...
    if (attribute === null) {
      const newAttr = attrGenerated.createImpl(this._globalObject, [], { localName: name, value });
  ...
  toggleAttribute(qualifiedName, force) {
    ...
    if (attribute === null) {
      if (force === undefined || force === true) {
        const newAttr = attrGenerated.createImpl(this._globalObject, [], { localName: qualifiedName, value: "" });
        attributes.appendAttribute(this, newAttr);
        return true;
      }
      return false;
