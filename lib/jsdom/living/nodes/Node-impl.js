...
const { CUSTOM_ELEMENT_REACTION_STACK, tryToUpgrade } = require("../helpers/custom-elements");
const {
  isShadowRoot, getRoot, shadowIncludingRoot, assignSlot, assignSlotableForTree, assignSlotable,
  shadowDomIncludingDescendantIterator, shadowDomIncludingInclusiveDescendantIterator,
...
class NodeImpl extends EventTargetImpl {
  ...
  _insert(nodeImpl, childImpl) {
    ...
    for (const node of nodesImpl) {
      ...
      // Optimization: Move the connected check from 7.7.2. out of the descendant iteration. if the parent is connected
      // then all the shadow dom inclusive descendants are connected as well.
      if (this._isConnected()) {
        for (const inclusiveDescendant of shadowDomIncludingInclusiveDescendantIterator(nodeImpl)) {
          // Optimization: Filter only on the elements nodes. Only elements are impacted by upgrade and custom element
          // reaction.
          if (inclusiveDescendant.nodeType === NODE_TYPE.ELEMENT_NODE) {
            if (inclusiveDescendant._isCustom()) {
              CUSTOM_ELEMENT_REACTION_STACK.enqueueCECallbackReaction(inclusiveDescendant, "connectedCallback", []);
            } else {
              tryToUpgrade(inclusiveDescendant);
            }
          }
        }
      }
  ...
  _remove(nodeImpl) {
    ...
    if (this._isConnected()) {
      if (nodeImpl.nodeType === NODE_TYPE.ELEMENT_NODE && nodeImpl._isCustom()) {
        CUSTOM_ELEMENT_REACTION_STACK.enqueueCECallbackReaction(nodeImpl, "disconnectedCallback", []);
      }

      for (const descendants of shadowDomIncludingDescendantIterator(nodeImpl)) {
        // Optimization: Filter only on the elements nodes. Only elements are impacted by upgrade and custom element
        // reaction.
        if (descendants.nodeType === NODE_TYPE.ELEMENT_NODE && descendants._isCustom()) {
          CUSTOM_ELEMENT_REACTION_STACK.enqueueCECallbackReaction(descendants, "disconnectedCallback", []);
        }
      }
    }
