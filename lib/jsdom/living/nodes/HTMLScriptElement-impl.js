...
class HTMLScriptElementImpl extends HTMLElementImpl {
  ...
  _poppedOffStackOfOpenElements() {
    ...
    if (this._ownerDocument._pendingParsingBlockingScript) {
      // TODO: Handle script nesting level

      const script = this._ownerDocument._pendingParsingBlockingScript;
      this._ownerDocument._pendingParsingBlockingScript = null;
      ...
      onLoad(data) {
        ...
        if (response && response.headers["content-type"]) {
          ...
          if (parsedContentType && parsedContentType.parameters.has("charset")) {
            characterEncoding = parsedContentType.parameters.get("charset");
          }
        }
