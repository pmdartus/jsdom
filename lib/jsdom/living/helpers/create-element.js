...
const { CUSTOM_ELEMENT_STATE, lookupCEDefinition } = require("./custom-elements");
...
class InterfaceCache {
  ...
  getByElementNamespaceAndName(namespace, name) {
    if (this._elementNamespaceNameLookup[namespace] && this._elementNamespaceNameLookup[namespace][name]) {
      return this._resolveEntry(this._elementNamespaceNameLookup[namespace][name]);
...
function createElement(document, localName, namespace, prefix = null, is = null, syncCustomElement = false) {
  ...
  if (definition && definition.name !== localName) {
    ...
    if (syncCustomElement) {
      ...
      // TODO: Enqueue a custom element upgrade reaction given result and definition.
    }
  } else {
