...
function findFlattenedSlotables(slot) {
  ...
  for (const node of slotables) {
    if (node.localName === "slot" && isShadowRoot(getRoot(node))) {
...
function findSlot(slotable, openFlag) {
  ...
  for (const child of domSymbolTree.treeIterator(shadow)) {
    if (child.localName === "slot" && child.name === slotable._slotableName) {
      return child;
    }
  }
...
function* shadowDomInclusiveDescendantIterator(node) {
  yield node;

  if (node._shadowRoot) {
    yield* shadowDomInclusiveDescendantIterator(node._shadowRoot);
  }

  for (const child of domSymbolTree.childrenIterator(node)) {
    yield* shadowDomInclusiveDescendantIterator(child);
  }
}
...
module.exports = {
  ...
  shadowDomInclusiveDescendantIterator
};
