...
exports.removeStylesheet = (sheet, elementImpl) => {
  const documentOrShadowRoot = nodeRoot(elementImpl);
  const { _sheets } = documentOrShadowRoot.styleSheets;
  _sheets.splice(_sheets.indexOf(sheet, 1));
...
function addStylesheet(sheet, elementImpl) {
  const documentOrShadowRoot = nodeRoot(elementImpl);
  documentOrShadowRoot.styleSheets._sheets.push(sheet);
