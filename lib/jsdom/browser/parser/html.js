...
function parseIntoDocument(markup, ownerDocument) {
  ...
  const parser = new Parse5Stream(config);

  parser.on("script", (scriptElement, documentWrite, resume) => {
    handlePendingParsingBlockScripts(ownerDocument, resume);
  });
...
function handlePendingParsingBlockScripts(ownerDocument, done) {
  if (!ownerDocument._pendingParsingBlockingScript) {
    done();
    return;
  }

  // TODO: Handle script nesting level

  const script = ownerDocument._pendingParsingBlockingScript;
  ownerDocument._pendingParsingBlockingScript = null;

  // TODO: Script level

  waitUntilScriptReadyToBeParserExecuted(script, () => {
    script._executeScriptBlock();
    handlePendingParsingBlockScripts(ownerDocument, done);
  });
}

function waitUntilScriptReadyToBeParserExecuted(script, done) {
  if (script._readyToBeParserExecuted) {
    done();
    return;
  }

  setTimeout(() => waitUntilScriptReadyToBeParserExecuted(script, done));
}
