...
function createDocumentType(ownerDocument, name, publicId, systemId) {
  ...
  return DocumentType.createImpl(_globalObject, [], { ownerDocument, name, publicId, systemId });
}
...
function createParser(rootNode, ownerDocument, saxesOptions) {
  ...
  parser.ontext = saxesOptions.fragment ?
    ...
    data => {
      appendChild(
        ...
        Text.createImpl(_globalObject, [], { data, ownerDocument })
      );
    } :
  ...
  parser.oncdata = data => {
    appendChild(
      openStack[openStack.length - 1],
      CDATASection.createImpl(_globalObject, [], { data, ownerDocument })
    );
  };
  ...
  parser.oncomment = data => {
    appendChild(
      openStack[openStack.length - 1],
      Comment.createImpl(_globalObject, [], { data, ownerDocument })
    );
  };
  ...
  parser.onprocessinginstruction = ({ target, body }) => {
    appendChild(
      openStack[openStack.length - 1],
      ProcessingInstruction.createImpl(_globalObject, [], { target, data: body, ownerDocument })
    );
  };
...
function parseFragment(markup, contextElement) {
  ...
  const fragment = DocumentFragment.createImpl(_globalObject, [], { _ownerDocument });

  // Only parseFragment needs resolvePrefix per the saxes documentation:
  // https://github.com/lddubeau/saxes#parsing-xml-fragments
  const parser = createParser(fragment, _ownerDocument, {
    xmlns: true,
    fragment: true,
