...
const { patchHTMLElementConstructors } = require("../living/helpers/create-element");
const { patchCEReactionProperties } = require("../living/helpers/ce-reaction");
...
function Window(options) {
  ...
  patchHTMLElementConstructors(window);
  patchCEReactionProperties(window);
  ...
  // TODO: Decide if we should or not create use create or createImpl?
  // A lot of custom elements tests use Proxy to track property access on the constructor. The CustomElementRegistry
  // wrapper tries to access the internal symbol on the constructor and make all the tests fails.
  const customElementRegistry = CustomElementRegistry.createImpl([], { ownerDocument: this._document });
