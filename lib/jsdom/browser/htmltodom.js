...
module.exports = class HTMLToDOM {
  ...
  parseFragment(markup, contextElement) {
    // TODO: improve lookup
    const algorithm = this.parser === parse5 ?
      this.parseFragmentHtml.bind(this) :
      this.parseFragmentXml.bind(this);

    const newChildren = algorithm(markup, contextElement);
    const fragment = DocumentFragment.createImpl([], {
      ownerDocument: contextElement._ownerDocument
    });

    for (const child of domSymbolTree.childrenIterator(newChildren)) {
      fragment._append(child);
    }

    return fragment;
  }

  parseFragmentHtml(input, contextNode) {
    const currentDocument = contextNode._ownerDocument;
    const treeAdapter = new JSDOMParse5Adapter(currentDocument);

    return this.parser.parseFragment(contextNode, input, {
      treeAdapter
    });
  }

  parseFragmentXml(input, contextNode) {
    const currentDocument = contextNode._ownerDocument;
    const parser = new this.parser.SaxesParser({
      xmlns: true,
      fragment: true,
    ...
    const openStack = [fragment];

    parser.ontext = text => {
      const textNode = currentDocument.createTextNode(text);
      openStack[openStack.length - 1]._append(textNode);
    };

    parser.oncdata = cdata => {
      const cdataNode = currentDocument.createCDATASection(cdata);
    ...
    parser.oncomment = comment => {
      const commentNode = currentDocument.createComment(comment);
      openStack[openStack.length - 1]._append(commentNode);
    };

    parser.onprocessinginstruction = ({ target, body }) => {
      const processingInstructionNode = currentDocument.createProcessingInstruction(target, body);
      openStack[openStack.length - 1]._append(processingInstructionNode);
    };

    parser.ondoctype = dt => {
      const doctypeNode = parseDocType(currentDocument, `<!doctype ${dt}>`);
      openStack[openStack.length - 1]._append(doctypeNode);
